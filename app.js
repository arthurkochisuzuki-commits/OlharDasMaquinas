...
class SecureVisionApp {
  ...
  /**
   * Alerta educativo caso o usuário abra o arquivo HTML diretamente (file://)
   * em navegadores que bloqueiam a webcam fora de um contexto seguro (localhost).
   */
  checkProtocolWarning() {
    if (window.location.protocol === 'file:') {
      const banner = document.createElement('div');
      banner.id = 'fileProtocolBanner';
      banner.style.cssText = 'position:fixed; top:0; left:0; right:0; z-index:99999; background:linear-gradient(90deg, #1e293b, #0f172a); border-bottom:1px solid #f59e0b; color:#f8fafc; padding:10px 16px; font-size:0.82rem; text-align:center; box-shadow:0 4px 16px rgba(0,0,0,0.6); display:flex; align-items:center; justify-content:center; gap:12px;';
      banner.innerHTML = `
        <span>💡 <strong>Dica de Compatibilidade:</strong> Para liberar as câmeras e webcams com 100% de permissão no navegador sem bloqueios de segurança (protocolo seguro), execute o arquivo <strong>index.bat</strong> ou <strong>INICIAR_PROJETO.bat</strong>.</span>
        <button style="background:rgba(245,158,11,0.2); border:1px solid #f59e0b; color:#f59e0b; border-radius:4px; padding:3px 12px; cursor:pointer; font-size:0.75rem; font-weight:600;" onclick="this.parentElement.remove()">OK, Entendi</button>
      `;
  ...
  async captureEnrollmentPhoto() {
    ...
    // Sistema sem limite de fotos de cadastro (captura ilimitada para amostragem estendida)
    ...
    this.enrollmentPhotos.push({ dataUrl, descriptor, canvas: isoCanvas });
    ...
    if (countEl) countEl.textContent = `${this.enrollmentPhotos.length} Fotos Biométricas Capturadas (Sem Limite)`;
  }
  ...
  clearEnrollmentPhotos() {
    ...
    const countEl = document.getElementById('sourcesCapturedCount');
    if (countEl) countEl.textContent = '0 Fotos Biométricas Capturadas (Sem Limite)';
  }
  ...
  async handleEnrollmentSubmit(e, isBlocked = false) {
    ...
    // Aumento de Dados Biométrico Automático (Espelhamento Horizontal, Iluminação e Rotação)
    const descriptors = [];
    const photoBlobs = [];

    for (const photo of this.enrollmentPhotos) {
      photoBlobs.push(photo.dataUrl);

      let srcCanvas = photo.canvas;
      if (!srcCanvas && photo.dataUrl) {
      ...
      if (srcCanvas && window.svBiometrics && window.svBiometrics.generateAugmentedFaceSamples) {
        const augmentedSamples = window.svBiometrics.generateAugmentedFaceSamples(srcCanvas);
        for (const sample of augmentedSamples) {
          descriptors.push(sample.descriptor);
          // Inclui a foto espelhada horizontalmente na galeria visual de fotos do banco
          if (sample.type === 'ESPELHAMENTO_HORIZONTAL') {
            photoBlobs.push(sample.canvas.toDataURL('image/jpeg'));
          }
        }
      } else if (photo.descriptor) {
        descriptors.push(photo.descriptor);
      }
    }

    // Regularização e Poda Anti-Overfitting (Elimina redundâncias e seleciona até 12 protótipos chave)
    const optimalDescriptors = (window.svBiometrics && window.svBiometrics.pruneAndRegularizeDescriptors)
      ? window.svBiometrics.pruneAndRegularizeDescriptors(descriptors, 12)
      : descriptors;
    ...
    try {
      await window.svDB.saveUser(
        ...
        { descriptors: optimalDescriptors, photoBlobs, videoBlob: null }
      );
      await window.svBiometrics.reloadRegisteredUsers();

      if (isBlocked) {
        alert(`🚫 PESSOA BLOQUEADA REGISTRADA!\n\n"${name}" foi registrado(a) na LISTA NEGRA.\n\n🧬 Aumento de Dados e Regularização Ativos:\n• Fotos originais capturadas: ${this.enrollmentPhotos.length}\n• Variações geradas: ${descriptors.length} amostras\n• Protótipos refinados no banco (Anti-Overfitting): ${optimalDescriptors.length} vetores de alta precisão.`);
      } else {
