...
class BiometricsEngine {
  ...
  async reloadRegisteredUsers() {
    try {
      ...
      for (const u of users) {
        ...
        // Regularização e Poda Anti-Overfitting: mantém até 12 protótipos de alta qualidade
        const regularizedDescriptors = this.pruneAndRegularizeDescriptors(rawDescriptors, 12);
        const centroidVector = this.aggregateVectorCentroid(regularizedDescriptors);
        ...
        loadedProfiles.push({
          ...
          isBlocked: isBlocked,
          descriptors: regularizedDescriptors,
  ...
  /**
   * REGULARIZAÇÃO E PODA BIOMÉTRICA (ANTI-OVERFITTING):
   * Quando muitas fotos ou aumentos de dados são gerados, este método:
   * 1. Elimina outliers (vetores com baixa correlação com o centróide próprio).
   * 2. Remove duplicatas redundantes (> 0.985 similaridade mútua).
   * 3. Seleciona os K protótipos mais representativos (máx 10-12), evitando que a fronteira
   *    de decisão infle e confunda estranhos com pessoas cadastradas.
   */
  pruneAndRegularizeDescriptors(descriptors, maxPrototypes = 12) {
    if (!descriptors || descriptors.length <= 1) return descriptors || [];

    // 1. Calcula o centróide global de referência de todas as fotos capturadas
    const rawCentroid = this.aggregateVectorCentroid(descriptors);
    if (!rawCentroid) return descriptors;

    // 2. Filtro de Coerência Anatômica (Poda de Ruído/Outliers)
    const candidates = [];
    for (const d of descriptors) {
      const normD = this.arcFace.l2Normalize(d);
      const simToCentroid = this.arcFace.computeCosine(normD, rawCentroid);
      // Descarta fotos com distorções, sombras severas ou artefatos que fujam da fisionomia
      if (simToCentroid >= 0.52) {
        candidates.push({ vec: normD, sim: simToCentroid });
      }
    }
    ...
    // Ordena os vetores pela maior representatividade/qualidade
    candidates.sort((a, b) => b.sim - a.sim);

    // 3. Deduplicação e Diversidade de Ângulos (Cosine Non-Maximum Suppression)
    const selectedPrototypes = [];
    for (const cand of candidates) {
      let isRedundant = false;
      for (const sel of selectedPrototypes) {
        const mutualCos = this.arcFace.computeCosine(cand.vec, sel);
        // Se já existe um vetor quase idêntico (> 98.5%), ignora para manter diversidade
        if (mutualCos > 0.985) {
          isRedundant = true;
          break;
        }
      ...
      if (!isRedundant) {
        selectedPrototypes.push(cand.vec);
        if (selectedPrototypes.length >= maxPrototypes) break;
      }
    }

    return selectedPrototypes;
  }

  /**
   * DATA AUGMENTATION BIOMÉTRICO (Aumento de Dados no Cadastro):
   * Espelha a face horizontalmente (inversão eixo X) e aplica variações anatômicas
   * de iluminação e rotação para enriquecer o banco de dados e garantir robustez.
   */
  generateAugmentedFaceSamples(sourceCanvas) {
    const width = sourceCanvas.width || 160;
    const height = sourceCanvas.height || 160;
    const samples = [];

    // 0. Amostra Original
    const origCtx = sourceCanvas.getContext('2d');
    samples.push({
      type: 'ORIGINAL',
      canvas: sourceCanvas,
      descriptor: this.extractDescriptorsFromImage(origCtx, width, height)
    });
    ...
    samples.push({
      type: 'ESPELHAMENTO_HORIZONTAL',
      canvas: flipCanvas,
      descriptor: this.extractDescriptorsFromImage(flipCtx, width, height)
    });

    // 2. ROTAÇÕES ANATÔMICAS LEVES (-5° e +5°)
    [-5, 5].forEach(angle => {
      ...
      rotCtx.drawImage(sourceCanvas, -width / 2, -height / 2);
      samples.push({
        type: `ROTACAO_${angle > 0 ? '+' : ''}${angle}DEG`,
        canvas: rotCanvas,
        descriptor: this.extractDescriptorsFromImage(rotCtx, width, height)
      });
    });

    // 3. VARIAÇÕES DE ILUMINAÇÃO (Preservando fundo 100% preto da máscara)
    const createBrightnessVariant = (baseCanvas, factor, offset, label) => {
      ...
      for (let i = 0; i < d.length; i += 4) {
        if (d[i + 3] > 30 && (d[i] > 5 || d[i + 1] > 5 || d[i + 2] > 5)) {
          d[i] = Math.max(0, Math.min(255, d[i] * factor + offset));
          d[i + 1] = Math.max(0, Math.min(255, d[i + 1] * factor + offset));
          d[i + 2] = Math.max(0, Math.min(255, d[i + 2] * factor + offset));
        }
      }
      ...
      return {
        type: label,
        canvas: bCanvas,
        descriptor: this.extractDescriptorsFromImage(bCtx, width, height)
      };
    };

    samples.push(createBrightnessVariant(sourceCanvas, 1.18, 10, 'ILUMINACAO_ALTA_+18%'));
    samples.push(createBrightnessVariant(sourceCanvas, 0.85, -8, 'ILUMINACAO_BAIXA_-15%'));
    samples.push(createBrightnessVariant(flipCanvas, 1.12, 6, 'ESPELHO_ILUMINACAO_+12%'));

    // 4. ESPELHAMENTO COMBINADO COM LEVE ROTAÇÃO (-4°)
    const flipRotCanvas = document.createElement('canvas');
    flipRotCanvas.width = width;
    flipRotCanvas.height = height;
    const frCtx = flipRotCanvas.getContext('2d');
    frCtx.translate(width / 2, height / 2);
    frCtx.rotate((-4 * Math.PI) / 180);
    frCtx.drawImage(flipCanvas, -width / 2, -height / 2);
    samples.push({
      type: 'ESPELHO_ROTACAO_-4DEG',
      canvas: flipRotCanvas,
      descriptor: this.extractDescriptorsFromImage(frCtx, width, height)
    });
  ...
  matchFaceArcFace(targetDescriptor, livenessResult = { isAlive: true, score: 0.85, status: 'LIVE_HUMAN_CONFIRMED' }) {
    ...
    for (const profile of this.registeredProfiles) {
      // 1. Similaridade com o Centróide Ponderado do usuário
      let centroidSim = -1.0;
      if (profile.weightCentroid) {
        centroidSim = this.arcFace.computeCosine(targetDescriptor, profile.weightCentroid);
        totalComparisons++;
      }

      // 2. Similaridades individuais com os protótipos regulares
      const sims = [];
      ...
      // 3. Média dos K melhores protótipos (Anti-Overfitting: elimina dependência de 1 único vetor anômalo)
      let topKSim = centroidSim;
      if (sims.length > 0) {
        const k = Math.min(3, sims.length);
        let sumTopK = 0;
        for (let i = 0; i < k; i++) sumTopK += sims[i];
        topKSim = sumTopK / k;
      }

      // 4. Escore Consensual Ponderado (55% Top-K + 45% Centróide)
      let consensusSim = 0;
      if (centroidSim > -1.0 && sims.length > 0) {
        consensusSim = (topKSim * 0.55) + (centroidSim * 0.45);
      } else if (centroidSim > -1.0) {
        consensusSim = centroidSim;
      } else if (sims.length > 0) {
        consensusSim = topKSim;
      }

      // 5. Filtro de Coerência Anatômica (Anti-Falso Positivo)
      // Se um estranho tiver coincidência espúria com 1 único vetor mas baixa correlação com o centróide,
      // penaliza o score para garantir que nunca seja confundido com a pessoa cadastrada.
      if (centroidSim > -1.0 && sims.length > 0 && (sims[0] - centroidSim > 0.25)) {
        consensusSim = Math.min(consensusSim, centroidSim + 0.08);
      }

      const marginResult = this.arcFace.computeArcMargin(consensusSim);

      if (consensusSim > maxCosine) {
        maxCosine = consensusSim;
